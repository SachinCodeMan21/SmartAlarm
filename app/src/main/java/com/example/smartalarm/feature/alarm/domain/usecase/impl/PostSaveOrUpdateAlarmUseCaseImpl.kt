 ...
 * It returns a [Result] containing a string that represents how long until the alarm triggers
...
class PostSaveOrUpdateAlarmUseCaseImpl @Inject constructor(
     ...
     * @return A [Result] containing a string representing the time until the alarm triggers,
     *         or an empty string if the alarm is disabled.
     */
    override fun invoke(alarm: AlarmModel): Result<String> {
...
//                )
                ...
                Result.Success(alarmTriggerTimeTextMessage)
            } else {
            ...
            // Catch any errors and return an error result with the exception
            Result.Error(e)
        }
